...
public class PaintPad extends View implements SharePadMgr.DataChangeListener, SharePadMgr.TopDataChangeListener {
    ...
    public void CheckBkImageSize() {
        ...
        if (bt != null) {
            float nimageHeight = bt.getHeight();
            float nimageWidth = bt.getWidth();
    ...
    boolean PaintBk(Canvas cvs) {
        ...
        if (isDisplayDraw) {
            ...
            if (bt != null && !bt.isRecycled()) {
                ...
                //默认最底层创建一层和白板相同颜色画布，防止透明图片显示教室背景图
                if (RoomControler.isCustomizeWhiteboard()) {
                    if (!TextUtils.isEmpty(WBSession.whiteboardcolorIndex)) {
                        int whiteboardcolorIndex = Integer.parseInt(WBSession.whiteboardcolorIndex) - 1;
                        if (whiteboardcolorIndex <= whiteBoardbackground.length) {
                            colorIndex = whiteboardcolorIndex;
                        }
                    }
    ...
    void PaintActions(Canvas cvs) {
        ...
        if (!isDisplayDraw) {
            if (getPadMgr().mCurrentShareDoc != null) {
                ...
                String strkey = showPageBean.getFiledata().getFileid() + "-" + showPageBean.getFiledata().getCurrpage();
                if (!getPadMgr().mSumLetty.isEmpty()) {
                    HashMap<String, List<TL_PadAction>> mSumLetty = (HashMap<String, List<TL_PadAction>>) getPadMgr().mSumLetty;
                    if (mSumLetty.containsKey(strkey)) {
                        List<TL_PadAction> padActions = mSumLetty.get(strkey);
                        for (int i = 0; i < padActions.size(); i++) {
                            if (!getPadMgr().mCurrentShareDoc.getFiledata().getFileid().equals(padActions.get(i).nDocID) || !(getPadMgr().mCurrentShareDoc.getFiledata().getCurrpage() == Integer.parseInt(padActions.get(i).nPage))) {
                                continue;
                            }
                            if (padActions.get(i).mClear != 0) {
                                continue;
                            }
                            if (!padActions.get(i).isDraw) {
                                continue;
                            }
    ...
    void PaintPadAction(TL_PadAction tl_pa, Canvas cvs) {
        switch (tl_pa.nActionMode) {
            case ft_line: {   //线
                ...
                if (mPaintPadActionUp != null && tl_pa.isNew && !tl_pa.inList) {
                    tl_pa.isNew = false;
...
//                linePaint.setAlpha(0);
    ...
    private float penWidthRatio() {
        ...
        if (getPadMgr().getCurrentImage() != null) {
            Bitmap bitmap = getPadMgr().getCurrentImage();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        if (mToolsType == ToolsType.defaule && (WhiteBoradConfig.getsInstance().getCurrentFileDoc().isDynamicPPT()
        ...
        switch (m_nActionMode) {
            ...
            case ft_Eraser: {
                if (m_tl_CurrentPadAction == null) {
                    ...
                    m_tl_CurrentPadAction.nDocID = getPadMgr().mCurrentShareDoc.getFiledata().getFileid();
                    m_tl_CurrentPadAction.nPage = String.valueOf(getPadMgr().mCurrentShareDoc.getFiledata().getCurrpage());
    ...
    public boolean OnTouchMove(MotionEvent event) {
        ...
        if (TKRoomManager.getInstance().getMySelf().canDraw && isMove && mToolsType == ToolsType.defaule) {
            return toMove(event);
        }

        if (m_nActionMode == null) return true;

        if (TKRoomManager.getInstance().getMySelf().canDraw && mToolsType != ToolsType.defaule) {
        ...
        switch (m_nActionMode) {
            ...
            case ft_Eraser: {
                if (m_tl_CurrentPadAction != null) {
                    ...
                    getPadMgr().topActions.add(m_tl_CurrentPadAction);
                    if (m_iSync != null)
    ...
    public void insertText(final float x, final float y) {
        ...
        inputWindowPop.showBoardPopupWindow(this, true, new OnSendClickListener() {
            @Override
        ...
        if (mEditTextInputControl != null) {
            mEditTextInputControl.showTextInput(x, y, (int) (m_nPenWidth / penWidthRatio() /** dbZoomScale*/), m_nPenColor);
        }

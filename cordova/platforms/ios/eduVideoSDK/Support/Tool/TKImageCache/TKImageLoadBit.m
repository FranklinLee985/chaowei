...
@interface TKImageLoadBit ()<NSURLSessionDataDelegate>
...
-(void)gifRequestSet:(NSString *)url{
    ...
    if (self.isNeedCache) {
        NSData *imgData = [TKImageCache getCacheImageData:[NSURL URLWithString:url] withCacheOptions:self.caCheTypeOption];
        if (imgData) {
...
-(void)gifRequestWith:(NSString *)url{
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
         ...
         */
        if (self.isNeedCache) {
            [TKImageCache cacheImageData:imgData withPath:[NSURL URLWithString:url]];
        }
        [self gifImageHandle:imgData withUrl:url];
    });
...
//    NSLog(@"%ld",data.length);
    ...
    if (self.TempImgData.length > self.expectedLeght/40 || self.expectedLeght == self.imgData.length) {
        ...
        if (self.isNeedCache && self.expectedLeght == self.imgData.length) {
            [TKImageCache cacheImageData:self.imgData withPath:dataTask.response.URL];
        }
        [self returnImageWith:img];
    }
}

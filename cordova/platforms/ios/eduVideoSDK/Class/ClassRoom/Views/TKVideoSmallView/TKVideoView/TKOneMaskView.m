...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _drawDotView = ({
            UIImageView *view = [[UIImageView alloc]init];
            view.backgroundColor = [colorid isEqualToString:TKOrangeSkin] ? RGBACOLOR(0, 0, 0, 0.5) : UIColor.clearColor;
...
- (void)setIVideoViewTag:(NSInteger)iVideoViewTag{
    ...
    self.turnOffCameraView.iVideoViewTag = iVideoViewTag;
    if (iVideoViewTag == -1) {
...
#pragma mark 视频画笔设置2
    ...
    if ([[dict allKeys] containsObject:sGiftNumber] && [[[TKEduSessionHandle shareInstance].roomMgr localUser].peerID isEqualToString:sFromId]) {
